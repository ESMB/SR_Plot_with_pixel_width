...
# Folders to analyse:
root_path=r"/Users/Mathew/Documents/Current analysis/Noelia/"
pathList=[]
...
pathList.append(r"/Users/Mathew/Documents/Current analysis/Noelia/515/")


pathList.append(r"/Users/Mathew/Documents/Current analysis/Noelia/638/")
...
def generate_SR_size(coords):
    box_size=10
    SR_prec_plot_def=np.zeros((image_width*40,image_height*40),dtype=float)
    dims=np.shape(SR_prec_plot_def)
    print(dims)
    j=0
    for i in coords:
        ...
        precisionx=5/Pixel_size*40
        precisiony=5/Pixel_size*40
        xcoord=coords[j,0]
        ycoord=coords[j,1]
        scale_xcoord=round(xcoord*40)
        scale_ycoord=round(ycoord*40)
...
def generate_SR_size_clu(coords,clusters):
    box_size=10
    SR_prec_plot_def=np.zeros((image_width*40,image_height*40),dtype=float)
    dims=np.shape(SR_prec_plot_def)
    print(dims)
...
for path in pathList:
    ...
    # Generate points SR (ESMB method):
    SRs=generate_SR_size(coords)
    imsr = Image.fromarray(SRs)
    imsr.save(path+filename_contains+'SR_points_with_width.tif')
    ...
    if to_cluster==1:
        ...
        if len(cluster_contents)>0:
            ...
            SR_clu_prec=generate_SR_size_clu(coords,clusters)
